...
echo_and_run(['cmake', '-B', 'build', '-DOpenJPEG_DIR={0}'.format(OPENJPEG_CMAKE_DIR), '-DCMAKE_TOOLCHAIN_FILE={0}'.format(os.path.join(os.environ.get('VCPKG_INSTALLATION_ROOT'), 'scripts/buildsystems/vcpkg.cmake')), '-DENABLE_CPP=OFF', '-DCMAKE_BUILD_TYPE=Release', '-DBUILD_QT5_TESTS=OFF', '.'], cwd = POPPLER_FOLDER)
