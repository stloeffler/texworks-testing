...
if [ "${TARGET_OS}" = "linux" -a "${TRAVIS_OS_NAME}" = "linux" ]; then
	...
	if [ "${QT}" -eq 4 ]; then
		...
		env PATH="${MXEDIR}/usr/bin:${MXEDIR}/usr/${MXETARGET}/qt/bin:$PATH" PREFIX="${MXEDIR}/usr" TARGET="${MXETARGET}" JOBS="$JOBS" MXE_CONFIGURE_OPTS="--host='$(TARGET)' --build='$(BUILD)' --prefix='$(PREFIX)/$(TARGET)' --enable-static --disable-shared ac_cv_prog_HAVE_DOXYGEN='false' --disable-doxygen" TEST_FILE="poppler-test.cxx" make -f build-poppler-mxe.mk
	elif [ "${QT}" -eq 5 ]; then
