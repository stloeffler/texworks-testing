...
if [ "${TARGET_OS}" = "linux" -a "${TRAVIS_OS_NAME}" = "linux" ]; then
	if [ "${QT}" -eq 5 ]; then
		...
		openssl aes-256-cbc -K $encrypted_9fc9b03c2815_key -iv $encrypted_9fc9b03c2815_iv -in "${TRAVIS_BUILD_DIR}/ci/travis-ci/launchpad/key.asc.enc" -out "${TRAVIS_BUILD_DIR}/ci/travis-ci/launchpad/key.asc" -d
